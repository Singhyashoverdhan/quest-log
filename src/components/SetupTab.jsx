...
export default function SetupTab({ ac, readOnly, selectedHabits, onToggleHabit }) {
  ...
  return (
    <div className="fade">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        ...
        {!readOnly && (
          <button
            ...
            style={{ fontSize: 12, color: '#A09C96', padding: '6px 12px', borderRadius: 20, border: '1px solid #EAE6DE', background: '#FFFFFF', cursor: 'pointer' }}
          >
      ...
      {/* Progress bar */}
      <div style={{ height: 4, background: '#F0EDE8', borderRadius: 4, marginBottom: 22, overflow: 'hidden' }}>
          ...
          {cats.map(cat => cat.activities.map(act => {
            ...
            return (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '11px 14px', borderRadius: 14, marginBottom: 7, border: '1px solid ' + (on ? col + '44' : '#EAE6DE'), background: on ? col + '0C' : '#FFFFFF', textAlign: 'left', transition: 'all .15s', cursor: readOnly ? 'default' : 'pointer', boxShadow: on ? 'none' : '0 1px 3px rgba(0,0,0,0.04)' }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                  <div style={{ width: 20, height: 20, borderRadius: 6, border: '1.5px solid ' + (on ? col : '#D8D4CC'), background: on ? col : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: '#FFFFFF', transition: 'all .15s' }}>
                    {on && I.Check(11)}
                  </div>
                  <div>
                    <div style={{ fontSize: 14, color: on ? '#1A1814' : '#A09C96', fontWeight: on ? 500 : 400 }}>{act.name}</div>
                    {act.trackTime && <div className="mono" style={{ fontSize: 10, color: '#C4C0BA', marginTop: 1 }}>timed · +{act.xp} xp</div>}
                  </div>
                </div>
                {!act.trackTime && <span className="mono" style={{ fontSize: 11, color: on ? col : '#C4C0BA' }}>+{act.xp}</span>}
              </button>
            );
          }))}
        </div>
