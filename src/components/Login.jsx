    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#FFFFFF' }}>
      ...
      {!sel ? (
        <div className="fade" style={{ width: '100%', maxWidth: 320 }}>
          ...
          {USERS.map(u => (
            <button key={u.name} onClick={() => { setSel(u.name); setPin(''); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '12px 16px', borderRadius: 14, marginBottom: 8, border: `1px solid ${u.color}44`, background: '#FFFFFF', color: '#1A1814', fontSize: 15, fontWeight: 600, boxShadow: '0 1px 3px rgba(0,0,0,0.05)', cursor: 'pointer' }}>
        ...
        <div className="fade" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: 280 }}>
          ...
          <div key={sk} className={err ? 'shake' : ''} style={{ display: 'flex', gap: 16, marginBottom: 36 }}>
            {[0, 1, 2, 3].map(i => <div key={i} style={{ width: 13, height: 13, borderRadius: '50%', background: pin.length > i ? (err ? '#C47878' : USERS.find(u => u.name === sel)?.color || ACCENT) : '#EAE6DE', transition: 'background .1s' }} />)}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, width: '100%' }}>
            {[1, 2, 3, 4, 5, 6, 7, 8, 9, '', 0, '⌫'].map((d, i) => (
              <button key={i} onClick={() => d === '' ? null : d === '⌫' ? setPin(p => p.slice(0, -1)) : digit(String(d))} disabled={d === ''}
                style={{ height: 58, borderRadius: 14, border: '1px solid #EAE6DE', background: d === '' ? 'transparent' : '#FFFFFF', color: '#1A1814', fontSize: 20, fontWeight: 600, opacity: d === '' ? 0 : 1, cursor: d === '' ? 'default' : 'pointer', boxShadow: d === '' ? 'none' : '0 1px 3px rgba(0,0,0,0.05)' }}>
                {d}
              </button>
            ))}
          </div>
